...
- **Full-screen editor**: `/dashboard/tours/:tourId/edit` moved out from under `DashboardLayout` in `src/app/routes.tsx` (still behind `RequireAuth`) — it's `fixed inset-0` and owns the whole viewport, matching how Userpilot/Chameleon's editor takes over the screen instead of living inside the dashboard chrome.
- **`EditorCanvas`** (`src/features/editor/components/EditorCanvas.tsx`) replaced the old dialog-only `ElementPickerCanvas`: a persistent URL bar + iframe is now the primary editing surface (not a modal), with a step chip strip on top (`SortableStepChip.tsx`, horizontal dnd-kit sort) and the dark `StepPropertiesPanel` docked right (wrapped in a `dark` class div — shadcn components already theme via CSS vars, so no per-component dark rewrite was needed).
- **Live positioning**: `public/picker.js` gained a `locate-element` mode — given a selector, it polls (`setInterval`, 150ms) `getBoundingClientRect()` and posts `locate-result` back. `useElementLocation.ts` consumes this to track a target element's live position; `StepOverlayPreview.tsx` renders the step card/highlight over the iframe using those coordinates, updating as the user types in the properties panel (`onLiveChange` callback threads live-edited title/type/content down from `StepPropertiesPanel` through `TourEditorPage` into the canvas, so the preview reflects unsaved edits, not just the last autosaved DB state).
- **Real bug found and fixed**: the overlay previously rendered unconditionally once a step was selected, silently falling back to a centered position whenever a target couldn't be located — giving no feedback about *why*. Now `StepOverlayPreview` explicitly branches: no selector set / snippet not detected on the loaded page / selector not found → distinct amber/red banners, only rendering the actual positioned card once a target is genuinely resolved. `EditorCanvas`'s toolbar also always shows a persistent "Snippet installed" / "Snippet not detected" indicator, and the "Pick element" button is hidden entirely until the snippet is confirmed present — no more implying picking works when it can't.
- **Install verification moved to project creation** (not tour creation, and not buried in the editor): `CreateProjectDialog` is now a 2-step flow — project details, then "Connect your site" (site URL → `useSnippetValidation.ts` loads it in a hidden iframe and waits for `picker.js`'s `picker-ready` message, with an 8s timeout → verified/failed feedback, "Recheck" or "Skip for now"). `projects` gained `site_url` and `is_installed` columns (`supabase/migrations/00000000000015_projects_site_install.sql`). The standalone Install page (`/dashboard/projects/:projectId/install`) is the ongoing reference: shows verification status with a "Recheck" button, the project's public key (generated on-demand, one active key per project — confirmed this is intentionally per-project, not shared workspace-wide, since different projects are typically different sites/domains), and both CDN `<script>` and npm install snippets.
- **SDK asset pipeline**: the CDN snippet references `/sdk/onboardflow.iife.js` relative to wherever the dashboard app is deployed. Added `scripts/copy-sdk-assets.mjs` + `predev`/`prebuild` npm hooks that build the `sdk/` workspace package and copy its `dist/` into `public/sdk/` so both `pnpm dev` and `pnpm build` serve it consistently. `public/sdk/` is gitignored (build artifact, not source, same reasoning as `dist/`).
- Verified end-to-end: full typecheck/lint/build clean; live Supabase test confirms `site_url`/`is_installed` write and the recheck update path both work; confirmed `dist/sdk/onboardflow.iife.js` exists after a production build.
